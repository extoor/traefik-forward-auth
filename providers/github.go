...
func (p *GitHubProvider) hasOrg(accessToken string) (bool, error) {
	...
	for _, org := range orgs {
		if p.Org == org.Login {
			log.Debugf("Found Github Organization: %q", org.Login)
			return true, nil
		}
...
func (p *GitHubProvider) hasOrgAndTeam(accessToken string) (bool, error) {
	...
	for _, team := range teams {
		...
		if p.Org == team.Org.Login {
			...
			for _, t := range ts {
				if t == team.Slug {
					log.Debugf("Found Github Organization:%q Team:%q (Name:%q)", team.Org.Login, team.Slug, team.Name)
					return true, nil
				}
			}
	...
	if hasOrg {
		log.Debugf("Missing Team:%q from Org:%q in teams: %v", p.Team, p.Org, presentTeams)
	} else {
		var allOrgs []string
		for org, _ := range presentOrgs {
			allOrgs = append(allOrgs, org)
		}
		log.Debugf("Missing Organization:%q in %#v", p.Org, allOrgs)
	}
	return false, nil
}
